...
from train_step import make_step_adam_prox, make_step_prox, make_step
from data_gen.data_gen_funs import six_variable_linear_func1, last_six_variable_linear_func2, six_variable_additive_func
...
def train(args):
    ...
    loader_key, model_key = jrand.split(key, 2)
    ...
    x, y, group = get_dataset(args.num_p, 1, [2000], func_list=[
        six_variable_linear_func1])
    ...
    for step, (xi, yi, groupi) in zip(range(args.max_iters), dataloader(
        ...
        if model.support() == 0:
            break
        all_loss, smooth_loss, unpen_loss, model, opt_state = make_step(
            model, optim, opt_state, xi, yi)
        if step % args.print_every == 0 or step == args.max_iters - 1:
            print(
                f"Step: {step}, All Loss: {all_loss}, Smooth Loss: {smooth_loss}, Unpen Loss: {unpen_loss}, support: {model.support()}")

    print("Stop training")
    print(f"Model support: {model.support()}")
